const DISCIPLINES = [
  ...
  {
    name: 'Laberinto',
    desc: 'Resolución autónoma de laberintos con algoritmos de exploración y mapeo en tiempo real.',
    icon: '⬕',
    color: 'wolf_green',
  },
  {
...
const ACCENT = {
  wolf_orange: { border: 'hover:border-wolf_orange/60', text: 'text-wolf_orange', bg: 'bg-wolf_orange' },
  wolf_blue:   { border: 'hover:border-wolf_blue/60',   text: 'text-wolf_blue',   bg: 'bg-wolf_blue'   },
  wolf_green:  { border: 'hover:border-wolf_green/60',  text: 'text-wolf_green',  bg: 'bg-wolf_green'  },
}
  ...
  return (
    <section id="disciplines" className="bg-wolf_dark py-24 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="reveal font-mono text-wolf_blue text-xs tracking-[0.4em] mb-3">[ 03 · DISCIPLINAS ]</p>
        <h2 className="reveal font-display font-bold text-4xl sm:text-5xl text-wolf_white mb-4" style={{ transitionDelay: '0.1s' }}>
          CATEGORÍAS DE<br /><span className="text-wolf_blue">COMPETENCIA</span>
        </h2>
        <p className="reveal font-body text-wolf_gray mb-16 max-w-xl" style={{ transitionDelay: '0.2s' }}>
          Participamos en múltiples disciplinas de robótica competitiva, cada una con sus propios desafíos técnicos y estratégicos.
        </p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {DISCIPLINES.map(({ name, desc, icon, color }, i) => {
            ...
            return (
              ...
              >
                <div className={`text-3xl ${a.text} mb-4 group-hover:scale-110 transition-transform duration-300`}>
                  {icon}
                </div>
                <h3 className={`font-display font-bold text-sm tracking-wider ${a.text} mb-3`}>{name.toUpperCase()}</h3>
                <p className="font-body text-wolf_gray text-sm leading-relaxed">{desc}</p>
                <div className={`mt-4 w-0 h-0.5 ${a.bg} group-hover:w-full transition-all duration-500`} />
              </div>
            )
          })}
        </div>
