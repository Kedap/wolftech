    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between">
        ...
        <button onClick={() => handleNav('#hero')} className="flex items-center gap-3 group">
          <WolfLogo variant="icon" size={36} className="group-hover:drop-shadow-[0_0_8px_#00D4FF] transition-all duration-300" />
        ...
        >
          <span className={`block w-6 h-0.5 bg-wolf_blue transition-all duration-300 ${menuOpen ? 'rotate-45 translate-y-2' : ''}`} />
          <span className={`block w-6 h-0.5 bg-wolf_blue transition-all duration-300 ${menuOpen ? 'opacity-0' : ''}`} />
          <span className={`block w-6 h-0.5 bg-wolf_blue transition-all duration-300 ${menuOpen ? '-rotate-45 -translate-y-2' : ''}`} />
        </button>
      </div>
